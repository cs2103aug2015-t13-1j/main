...
public class CommandParser {
  ...
  // because of the way tokenizing is done, an argument like date and time which has a space between them is counted as 2 arguments
	...
	// positions in the parameter list for the relocate command
	private static final int POSITION_RELOCATE_DIRECTORY = 0;
  ...
  private static Command initRelocateCommand(ArrayList<String> args) throws Exception {
  	...
  	String directory = "";
		...
		if (args.size() >= 1) {
			directory = args.get(POSITION_RELOCATE_DIRECTORY); // name is always present in the same position for all tasks
		}
		
		newTask = new Task(directory, false);
  	
  	return new Relocate(newTask);
  }
